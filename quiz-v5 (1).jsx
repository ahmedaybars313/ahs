...
export default function App(){
  ...
  function goHome(){setTimerActive(false);setHistoryIdx(null);setScreen("intro");}
  function startMode(m){setMode(m);setHistory([]);setHistoryIdx(null);setRetryQueue([]);setQuestionCount(0);setTimerSec(0);setTimerActive(true);setKazStats({});loadNext(m);}
  ...
  function loadNext(m,newCount){
    ...
    const dueRetry=retryQueue.find(r=>r.dueAt<=cnt);
    if(dueRetry){
      setRetryQueue(prev=>prev.filter(r=>r!==dueRetry));
      setIsRetry(true);
      setQ(dueRetry.q);setSel(null);setSimplified(false);setHistoryIdx(null);setScreen("question");
      return;
    }
    const nq=pickQ(levels,used,curMode);
    if(!nq&&!retryQueue.length){setScreen("done");return;}
  ...
  function goForward(){
    ...
    if(historyIdx>=history.length-1){setHistoryIdx(null);setScreen("question");}
    else setHistoryIdx(historyIdx+1);
  }
  function resetAll(m){
    setUsed(new Set());
    setLevels(()=>{const l={};for(let i=1;i<=12;i++)l[i]=1;for(let i=101;i<=110;i++)l[i]=1;return l;});
    setStats({c:0,t:0,wrong:0});setStreak(0);setHistory([]);setHistoryIdx(null);setRetryQueue([]);setQuestionCount(0);setTimerSec(0);setTimerActive(true);setKazStats({});setMode(m);loadNext(m,0);
  }
  function fullReset(){
    setUsed(new Set());
    setLevels(()=>{const l={};for(let i=1;i<=12;i++)l[i]=1;for(let i=101;i<=110;i++)l[i]=1;return l;});
    setStats({c:0,t:0,wrong:0});setStreak(0);setHistory([]);setHistoryIdx(null);setRetryQueue([]);setQuestionCount(0);setTimerSec(0);setTimerActive(false);setKazStats({});setMode(null);setScreen("intro");setShowStats(false);
  }
      ...
      <div style={S.wrap}>
        <div style={S.card}>
          ...
          {stats.t>0&&(
            <>
              ...
              <div style={{marginTop:10,padding:"8px 12px",background:"#f8fafc",borderRadius:10,fontSize:12,color:"#374151"}}>
                <b>Mod:</b> {mode?SUBJ_LABEL[mode]||"🎲 Karışık":"—"} · <b>Çözülen:</b> {solvedInMode}/{totalInMode}
                {streak>=2&&<> · <b>🔥{streak}</b></>}
              </div>
            </>
          )}
          ...
          <div style={{display:"flex",gap:8,marginTop:14}}>
            <button style={{...S.btn("#1d4ed8"),marginTop:0,flex:2}} onClick={()=>{setShowStats(false);if(mode)continueMode();}}>
              ▶ Devam Et
            </button>
    ...
    return(
      <div style={S.app}>
        <div style={{...S.hdr(rColor),display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px"}}>
          ...
          <button onClick={()=>{setHistoryIdx(null);setScreen("question");}} style={{background:"rgba(255,255,255,.2)",
        ...
        <div style={S.wrap}>
          ...
          <div style={{display:"flex",gap:8,marginTop:4,paddingBottom:20}}>
            ...
            <button onClick={()=>!isLast?setHistoryIdx(i=>i+1):(setHistoryIdx(null),setScreen("question"))}
  ...
  if(screen==="intro") return(
    <div style={S.app}>
      <div style={S.hdr("#b91c1c")}>
        <p style={S.hdrT}>🎓 Ahmed Hilmi'nin Soru Bankası</p>
        <p style={S.hdrS}>Akıllı Adaptif Sistem · {QB.length} Soru</p>
      </div>
      <div style={S.wrap}>
        {hasProgress&&(
          <div style={{background:"#fff",borderRadius:14,boxShadow:"0 2px 10px rgba(0,0,0,.08)",
            padding:"14px 16px",marginTop:14,border:"2px solid #d1fae5"}}>
            <div style={{fontSize:12,color:"#16a34a",fontWeight:"bold",marginBottom:8}}>📌 Kaldığın Yer</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:12}}>
              {[
                {label:"Çözülen",val:stats.t,c:"#1d4ed8"},
                {label:"Doğru",val:stats.c,c:"#16a34a"},
                {label:"Yanlış",val:stats.wrong,c:"#dc2626"},
                {label:"Kalan",val:remaining,c:"#d97706"},
              ].map(({label,val,c})=>(
                <div key={label} style={{textAlign:"center"}}>
                  <div style={{fontSize:18,fontWeight:"bold",color:c}}>{val}</div>
                  <div style={{fontSize:10,color:"#6b7280"}}>{label}</div>
                </div>
              ))}
            </div>
            {stats.t>0&&<div style={S.pbar}><div style={S.pbarFill(pct,pct>=70?"#16a34a":pct>=50?"#d97706":"#dc2626")}/></div>}
            <div style={{display:"flex",gap:8,marginTop:10}}>
              <button style={{...S.btn("#16a34a"),marginTop:0,flex:2}} onClick={continueMode}>
                ▶ Kaldığım Yerden Devam Et
              </button>
              <button style={{...S.btn("#94a3b8"),marginTop:0,flex:1,fontSize:12}} onClick={fullReset}>
                🔄 Sıfırla
              </button>
            </div>
          </div>
        ...
        <div style={S.card}>
          {!hasProgress&&(
            <div style={{textAlign:"center",padding:"8px 0 14px"}}>
              ...
              <p style={{color:"#64748b",fontSize:13,lineHeight:1.6,margin:"4px 0 0"}}>
                Doğru yaparsan sorular zorlaşır 📈<br/>
                Yanlış yaparsan konu anlatılır ve soru basite bölünür 🧩
              </p>
            </div>
          )}
          <p style={{fontWeight:"bold",fontSize:13,color:"#374151",marginBottom:10,textAlign:"center",marginTop:hasProgress?0:14}}>
            {hasProgress?"Yeni bir ders seç:":"Hangi dersten başlamak istersin?"}
          </p>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
            {[
              ...
              const done=[...used].filter(id=>{const x=QB.find(y=>y.id===id);return x&&x.subject===subj;}).length;
              const pctS=total?Math.round(done/total*100):0;
              return(
                <button key={subj} style={{...S.subjBtn(color,false),flex:"1 1 45%",position:"relative"}} onClick={()=>resetAll(subj)}>
                  {icon}<br/><span style={{fontSize:15}}>{label}</span><br/>
                  <span style={{fontSize:10,opacity:.8}}>{done}/{total}</span>
                  {done>0&&<div style={{position:"absolute",bottom:4,left:4,right:4,height:3,background:"rgba(0,0,0,.1)",borderRadius:2,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${pctS}%`,background:color,borderRadius:2}}/>
                  </div>}
                </button>
              );
            })}
          </div>
          <button style={{...S.subjBtn("#7c3aed",false),width:"100%"}} onClick={()=>resetAll("karisik")}>
            🎲 Karışık — Tüm dersler ({QB.length} soru)
          </button>
          {hasProgress&&(
            <button style={{...S.btn("#1a1a2e"),marginTop:10,fontSize:13}} onClick={()=>setShowStats(true)}>
      ...
      <div style={S.wrap}>
        <div style={S.card}>
          ...
          <div style={{display:"flex",gap:8,marginTop:8}}>
            <button style={{...S.btn("#b91c1c"),marginTop:0,flex:1}} onClick={()=>resetAll("turkce")}>📖</button>
            <button style={{...S.btn("#1d4ed8"),marginTop:0,flex:1}} onClick={()=>resetAll("matematik")}>🔢</button>
            <button style={{...S.btn("#16a34a"),marginTop:0,flex:1}} onClick={()=>resetAll("fen")}>🔬</button>
            <button style={{...S.btn("#d97706"),marginTop:0,flex:1}} onClick={()=>resetAll("sosyal")}>🌍</button>
            <button style={{...S.btn("#7c3aed"),marginTop:0,flex:2}} onClick={()=>resetAll("karisik")}>🎲 Karışık</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div style={S.wrap}>
        ...
        <div style={{background:"#fff",borderRadius:13,boxShadow:"0 2px 10px rgba(0,0,0,.08)",padding:"11px 14px",marginTop:12,border:`2px solid ${hdrColor}33`}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:4}}>
            ...
            <span style={{fontSize:11,color:"#94a3b8"}}>{solvedInMode+1}/{totalInMode}</span>
          </div>
